    ...
    for (index, element) in _document.elements.iter().enumerate() {
        if let DocumentElement::Image { description, .. } = element {
            descriptions.push(ImageDescription {
                image_index: index,
                description: format!(
                    "AI-generated description for: {description}"
                ),
                confidence: 0.85,
            });
        }
...
pub async fn analyze_risks(
    ...
    Ok(vec![RiskItem {
        ...
        description:
            "Unusual payment terms detected that may pose financial risk."
                .to_string(),
        ...
        match self.provider.as_str() {
            ...
            "openai" => {
                let api_key = self.api_key.clone().ok_or_else(|| {
                    anyhow::anyhow!("OpenAI API key required")
                })?;
                Ok(AIProvider::OpenAI(api_key))
            }
            "anthropic" | "claude" => {
                let api_key = self.api_key.clone().ok_or_else(|| {
                    anyhow::anyhow!("Anthropic API key required")
                })?;
