        ...
        match element {
            ...
            DocumentElement::List { items, ordered } => {
                for (i, item) in items.iter().enumerate() {
                    ...
                    markdown
                        .push_str(&format!("{indent}{bullet}{item_text}\n"));
                }
                markdown.push('\n');
            }
    ...
    for element in &document.elements {
        match element {
            ...
            DocumentElement::Table { table } => {
                ...
                // Top border
                let top_border =
                    generate_text_table_border(col_widths, "┌", "┬", "┐", "─");
                text.push_str(&format!("{top_border}\n"));

                // Header with proper alignment
                let header_line =
                    render_text_table_row(&table.headers, col_widths, true);
                ...
                for row in &table.rows {
                    let row_line =
                        render_text_table_row(row, col_widths, false);
    ...
    for (index, element) in document.elements.iter().enumerate() {
        if let DocumentElement::Heading { text, .. } = element {
                ...
                || text.to_lowercase().contains("works cited")
            {
                ...
                for (bib_index, bib_element) in
                    document.elements[index + 1..].iter().enumerate()
                {
                    match bib_element {
...
fn render_text_table_row(
    ...
    for (i, cell) in cells.iter().enumerate() {
        ...
        let aligned_content =
            align_text_cell_content(&cell.content, cell.alignment, width);
